...
class FloorPlaneMappingBayes {
	  ...
	  // push point cloud points into our matrix at the appropriate index. 
            ...
            for (int i=0; i<n_x; i++) {
              for (int j=0; j<n_y; j++) {
                ...
                if (n_list < 30) {continue;}

		double minimum_z = 10;
		double maximum_z = 0;
		for (PointList::const_iterator it=map_array[i][j].begin();it != map_array[i][j].end(); it++) {
		  const pcl::PointXYZ & P = *it;
		  if (P.z < minimum_z){minimum_z = P.z;}
		  if (P.z > maximum_z){maximum_z = P.z;}
		}	
		double height_spread = maximum_z - minimum_z;
		if (height_spread > n_x/10){ // obstacle 
    ...
    public:
        FloorPlaneMappingBayes() : nh_("~"), it_(nh_) {
            ...
            scan_sub_ = nh_.subscribe("scans", 1, &FloorPlaneMappingBayes::pc_callback, this);
...
{
    ros::init(argc,argv,"floor_plane_mapping_bayes");
    FloorPlaneMappingBayes fp;
